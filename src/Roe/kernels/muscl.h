...
void U2FpFd_AVG(double U_L[4], double U_R[4], double F_[4]) {
  ...
  lambda4p = 0.5 * (lambda4p + sqrt(lambda4p * lambda4p + 1e-16));
  */
  // lambda1p = 0.5 * (0.5 * (fabs(lambda1) + lambda1) + sqrt(0.25 * (fabs(lambda1) + lambda1) * (fabs(lambda1) +
  // lambda1) + 1e-16));
  ...
  f3 = rou / 2.0 / GAMA * (2 * ((GAMA - 1) * H - a * a) * lambda1p + (H - a * u) * lambda3p + (H + a * u) * lambda4p);
  //现在就算Fd即F-
  rou = U_R[0];
  u = U_R[1] / U_R[0];
  v = U_R[2] / U_R[0];
...
void U2GpGd_AVG(double U_L[4], double U_R[4], double G_[4]) {
  ...
  */
  ...
  g3 = rou / 2.0 / GAMA * (2 * ((GAMA - 1) * H - a * a) * lambda1p + (H - a * v) * lambda3p + (H + a * v) * lambda4p);
  //计算Gd即G-的函数
  rou = U_R[0];
  u = U_R[1] / U_R[0];
  v = U_R[2] / U_R[0];
