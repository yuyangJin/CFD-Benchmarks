...
void SymTVD_x(double U[Nx + 7][Ny + 7][4], double U_[Nx + 7][Ny + 7][4], double a_[Nx + 7][Ny + 7][1],
              ...
              double Q_[Nx + 7][Ny + 7][4], double theta[Nx + 7][Ny + 7][4], double F[Nx + 7][Ny + 7][4],
              double F_[Nx + 7][Ny + 7][4], double dx, double dy,
              double &dt)  //此函数将之前计算的特征之余特征向量通过TVD算法计算U
{
  ...
  for (i = 2; i <= Nx + 3; i++)
    for (j = 2; j <= Ny + 3; j++)
      if (U[i][j][0] != 0)
        for (k = 0; k <= 3; k++) g_[i][j][k] = minmod3(alpha_[i - 1][j][k], alpha_[i][j][k], alpha_[i + 1][j][k]);
  //计算Q_
  for (i = 2; i <= Nx + 3; i++)
    for (j = 2; j <= Ny + 3; j++)
      if (U[i][j][0] != 0)
...
void SymTVD_y(double U[Nx + 7][Ny + 7][4], double U_[Nx + 7][Ny + 7][4], double a_[Nx + 7][Ny + 7][1],
              double LAMDA_[Nx + 7][Ny + 7][4][4], double L_[Nx + 7][Ny + 7][4][4], double R_[Nx + 7][Ny + 7][4][4],
              double alpha_[Nx + 7][Ny + 7][4], double g_[Nx + 7][Ny + 7][4], double gama_[Nx + 7][Ny + 7][4],
              double Q_[Nx + 7][Ny + 7][4], double theta[Nx + 7][Ny + 7][4], double G[Nx + 7][Ny + 7][4],
              double G_[Nx + 7][Ny + 7][4], double dx, double dy,
              double &dt)  //此函数将之前计算的特征之余特征向量通过TVD算法计算U
{
  int i, j, k, l;
  double r, z;
  ...
  for (i = 2; i <= Nx + 3; i++)
    for (j = 2; j <= Ny + 3; j++)
      if (U[i][j][0] != 0)
        for (k = 0; k <= 3; k++) g_[i][j][k] = minmod3(alpha_[i][j - 1][k], alpha_[i][j][k], alpha_[i][j + 1][k]);
  //计算Q_
  for (i = 2; i <= Nx + 3; i++)
    for (j = 2; j <= Ny + 3; j++)
      if (U[i][j][0] != 0)
  ...
  for (i = 1; i <= Nx + 4; i++)
    for (j = 1; j <= Ny + 4; j++)
      if (U[i][j][0] != 0) {
        ...
        for (k = 0; k <= 3; k++) {
          double _a = 0;
          for (l = 0; l <= 3; l++) _a += local_L_[k][l] * (U[i + 1][j][l] - U[i][j][l]);
          alpha_[i][j][k] = _a;
        }
      }
    ...
    for (j = 2; j <= Ny + 3; j++)
      if (U[i][j][0] != 0) {
        ...
        for (k = 0; k <= 3; k++) {
          ...
          local_g_[k] = minmod3(alpha_[i - 1][j][k], alpha_[i][j][k], alpha_[i + 1][j][k]);
        }
  ...
  for (i = 0; i <= Nx + 5; i++)
    for (j = 0; j <= Ny + 5; j++)
      if (U[i][j][0] != 0) {
        ...
        for (k = 0; k <= 3; k++) {
          double _a = 0;
          for (l = 0; l <= 3; l++) _a += local_L_[k][l] * (U[i][j + 1][l] - U[i][j][l]);
          alpha_[i][j][k] = _a;
        }
      }
      ...
      if (U[i][j][0] != 0) {
        ...
        for (k = 0; k <= 3; k++) {
          ...
          if (fabs(z) < DELTA) {
            ...
            local_Q_[k] = fabs(z);
          local_g_[k] = minmod3(alpha_[i][j - 1][k], alpha_[i][j][k], alpha_[i][j + 1][k]);
        }
