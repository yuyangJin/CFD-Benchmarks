...
void bound(double U[I+2][J+2][4])
{
	...
	U[i][i-28][2]=U[i][i-28][1];	U[i][i-28][3]=(U[i-1][i-27][3]+U[i+1][i-29][3])*0.5;
...
/*计算f+*/
void U2EA(double U[I+2][J+2][4],double EA[I+2][J+2][4],int i,int j)
{
	...
	for(s=0;s<=3;s++)
	{
        UL3[i][j][s]=U[i][j][s]+0.25*(1-k)*(minmod((U[i][j][s]-U[i-1][j][s]),(b*(U[i+1][j][s]-U[i][j][s]))))
	...
	EA[i][j][0]=rou/2/GAMA*(2*(GAMA-1)*f1+f2+f3);	EA[i][j][1]=rou/2/GAMA*(2*(GAMA-1)*u*f1+(u-a)*f2+(u+a)  *f3);
...
/*计算f-*/
void U2EB(double U[I+2][J+2][4],double EB[I+2][J+2][4],int i,int j)
{
	...
	for(s=0;s<=3;s++)
	{
        UR3[i][j][s]=U[i+1][j][s]-0.25*(1-k)*(minmod((U[i+2][j][s]-U[i+1][j][s]),(b*(U[i+1][j][s]-U[i][j][s]))))
	...
	EB[i][j][0]=rou/2/GAMA*(2*(GAMA-1)*f1+f2+f3);	EB[i][j][1]=rou/2/GAMA*(2*(GAMA-1)*u*f1+(u-a)*f2+(u+a)*f3);	EB[i][j][2]=rou/2/GAMA*(2*(GAMA-1)*f1+f2+f3)*v;	EB[i][j][3]=rou/2/GAMA*((GAMA-1)*(u*u+v*v)*f1+(h-a*u)*f2+(h+a*u)*f2);
	return;
}

/*计算g+*/
void U2FA(double U[I+2][J+2][4],double FA[I+2][J+2][4],int i,int j)
 {
	...
	for(s=0;s<=3;s++)
	{
        ...
        UL4[i][j][s]=U[i][j][s]+0.25*(1-k)*(minmod((U[i][j][s]-U[i][j-1][s]),(b*(U[i][j+1][s]-U[i][j][s]))))
...
/*计?算g-*/
void U2FB(double U[I+2][J+2][4],double FB[I+2][J+2][4],int i,int j)
{
	double rou,u,v,p,a,h,g1,g2,g3;
	double DD,pL,pR,hL,hR;
	int s=0;
	for(s=0;s<=3;s++)
	{
		UR4[i][j][s]=U[i][j+1][s]-0.25*(1-k)*(minmod((U[i][j+2][s]-U[i][j+1][s]),(b*(U[i][j+1][s]-U[i][j][s]))))
			...
			for(j=2;(j<=J-1);j++)
			{
				if(j>=i-28)
				{
					if(U[i][j][0]!=0)
				  {
					...
					for(s=0;s<=3;s++)
				    {   
					   U[i][j][s]=U[i][j][s]-n/dx*((EA[i][j][s]+EB[i][j][s])-(EA[i-1][j][s]+EB[i-1][j][s]))-n/dx*((FA[i][j][s]+FB[i][j][s])-(FA[i][j-1][s]+FB[i][j-1][s])) ; ;
				    }
