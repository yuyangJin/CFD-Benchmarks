...
//全局变量
int N_x,N_y;
double UF[Jx+5][Jy+5][4],UG[Jx+5][Jy+5][4];
double U[Jx+5][Jy+5][4],F[Jx+5][Jy+5][4],G[Jx+5][Jy+5][4];
double rou[Jx+5][Jy+5],u[Jx+5][Jy+5],v[Jx+5][Jy+5],h[Jx+5][Jy+5],p[Jx+5][Jy+5];
...
计算流场
入口：U矢量
出口：rou[Jx+5][Jy+5],u[Jx+5][Jy+5],v[Jx+5][Jy+5],h[Jx+5][Jy+5],p[Jx+5][Jy+5]
--------------------------------------------------------*/
void Calculate_field(double U[Jx+5][Jy+5][4],double rou[Jx+5][Jy+5],double u[Jx+5][Jy+5],double v[Jx+5][Jy+5],double h[Jx+5][Jy+5],double p[Jx+5][Jy+5])
{
	int i,j;
	for(i=0;i<=Jx+4;i++)
		for(j=0;j<=Jy+4;j++)
...
//计算Fai[Jy+5][4]
		for(j=0;j<=Jy+4;j++)
			for(k=0;k<=3;k++)
				for(l=0;l<=3;l++)
					Fai[j][k]=Fai[j][k]-R[j][k][l]*(pow(ry*lamda[j][l],2)*g_b[j][l]+Q_z(ry*lamda[j][l])*(alfa[j][l]-g_b[j][l]))/ry;
	...
	for(i=2;i<=Jx+2;i++)
		for(j=2;j<=Jy+2;j++)
			for(k=0;k<=3;k++)
			{
				if(i>N_x+1&&j<N_y-1)
					continue;
				U[i][j][k]=U[i][j][k]-rx*(F[i][j][k]-F[i-1][j][k])-ry*(G[i][j][k]-G[i][j-1][k]);
			}
